...
export const buildDailyPuzzleGames = (
  ...
  Array.from({ length: batchSize }).forEach((_, i) => {
    ...
    let result: number[];
    try {
      result = shufflePieces(isWeekend);
    } catch (e) {
      addWarning(
        'vitrais',
 ...
 */
function shufflePieces(isWeekend: boolean) {
  const piecesCount = isWeekend ? sample([18, 21, 24]) : sample([12, 15]);
...
// export const buildDailyPuzzleGamesLegacy = (
//   batchSize: number,
//   history: ParsedDailyHistoryEntry,
//   puzzleSets: Dictionary<DailyPuzzleSet>,
// ) => {
...
//     if (id === '2026-01-01') {
//       selectedSet = puzzleSets['td-d5-145']; // Special set for New Year 2026
//     }
